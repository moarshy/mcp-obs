...
MCPlatform provides **infrastructure + SDK** for dev companies who build their own MCP servers:

```
┌─────────────────────────────────────────────────────────────┐
│              MCPlatform (Infrastructure)                    │
│                                                             │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐  │
...
**MCPlatform Infrastructure (Next.js App)**
...
**MCPlatform SDK (Dual Integration)**
...
- **Analytics Reporting**: Sends telemetry data to MCPlatform

**Client-Side SDK (for MCP Clients)**
- **OAuth Client**: Handles authentication flows with MCPlatform
...
2. Integrates MCPlatform SDK into their MCP server
3. SDK handles all auth + analytics automatically
4. Customer configures everything via MCPlatform dashboard
...
1. **Sign Up & Organization**:
   ```
   - Creates account at mcplatform.com with GitHub OAuth
...
2. **MCP Server Registration**:
   ...
   Slug: "docuapi" → Creates docuapi.mcplatform.com
...
3. **What They Get**:
   - **Subdomain**: `docuapi.mcplatform.com` for MCP connections
...
1. **Install MCPlatform SDK**:
   ```bash
   npm install @mcplatform/sdk
   ```
...
2. **Integrate SDK into their MCP Server**:
   ...
   // NEW: With MCPlatform SDK
   import { Server } from '@modelcontextprotocol/sdk/server/index.js'
   import { MCPlatformSDK } from '@mcplatform/sdk'

   const server = new Server()
   const mcpPlatform = new MCPlatformSDK({
     serverSlug: 'docuapi',
     apiKey: process.env.MCPLATFORM_API_KEY
   })
...
3. **Client Documentation Update** (Enhanced for SDK):
   ...
   ## Option 1: With MCPlatform Client SDK (Recommended)
   ```typescript
   import { MCPlatformClient } from '@mcplatform/client-sdk'

   const client = new MCPlatformClient({
     serverUrl: 'https://api.docuapi.com/mcp',
     authDomain: 'docuapi.mcplatform.com',
     telemetry: {
       enabled: true,
   ...
   - Users authenticate via MCPlatform OAuth
   - All analytics captured transparently
   ```

4. **What Changes**:
   - **MCP Server**: Adds server-side SDK middleware + instrumentation
   - **Client Integration**: Either use client SDK or configure MCP client for MCPlatform auth
...
#### What DocuAPI Gets From MCPlatform:
...
**Context**: Marcus needs to integrate MCPlatform observability with their existing Datadog setup.

**Integration Flow**:
1. **OTLP Configuration**: Configures MCPlatform to export to Datadog OTLP endpoint
...
**Connection Flow**:
1. **MCP Client Connection**: Claude Desktop connects to `docuapi.mcplatform.com`
2. **Authentication Required**: MCPlatform detects no auth, redirects to OAuth flow
3. **OAuth Login**: Alex chooses "Sign in with GitHub"
4. **Consent**: Grants permission for DocuAPI to access basic profile info
5. **Session Creation**: MCPlatform creates session, issues tokens
...
- **Instrumentation**: MCPlatform captures:
  ```
...
**Handles**:
- Customer signup/login at mcplatform.com
...
export const auth = betterAuth({
  ...
  trustedOrigins: ['*.mcplatform.com'],  // Cross-subdomain cookies
  socialProviders: {
    google: { /* config */ },
    github: { /* config */ }
  },
...
**Handles**:
- End-user OAuth at {slug}.mcplatform.com
...
**Option 2: MCPlatform OAuth (Recommended)**
```typescript
// MCP Server Config
{
...
- **MCPlatform as OAuth Server**: We provide the complete OAuth infrastructure
- **User Experience**: Login at docuapi.mcplatform.com/mcp-oidc/login
...
```typescript
// MCPlatform Client SDK automatically instruments
...
```typescript
// MCPlatform Server SDK receives trace context and continues span
...
Client SDK → MCPlatform Collector → Customer OTLP Endpoint

// Server telemetry (business logic, tool usage)
Server SDK → MCPlatform Collector → Customer OTLP Endpoint
...
```typescript
{
  ...
  journey: {
    ...
    authLatency: 15,        // MCPlatform auth
...
import { router } from '@mcplatform/server'
...
### Month 1: Foundation
- [ ] Strip down existing MCPlatform to core auth proxy
...
This pivot leverages MCPlatform's existing technical infrastructure while addressing real market pain points. The combination of authentication proxy and OpenTelemetry-based observability creates a defensible, enterprise-ready product that can scale with the growing MCP ecosystem.
