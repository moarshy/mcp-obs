...
import { revokeToken } from '@/lib/mcp-oauth/token-exchange'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Client authentication is optional for token revocation per RFC 7009
    ...
    if (!revocationResult.success) {
      return NextResponse.json(
        {
          error: 'server_error',
          error_description: revocationResult.error || 'Failed to revoke token'
        },
        { status: 500 }
      )
    }
...
// OPTIONS /oauth/revoke - CORS preflight
